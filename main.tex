...
The dataset is composed of 4613 datapoints. Each datapoint is represented as a tuple $(M, a, ate, are)$, where:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Rescaling: after the logarithmic rescaling, a linear rescaling to $[0,1]$ has been applied; let
        ...
        \begin{figure}[h]
            ...
            \end{minipage}
            \vspace{-1em}
            \caption{Dataset values after the log and linear rescaling.}
        \end{figure}
    ...
    \item Image cropping: each map has been cropped to remove unnecessary borders, focusing only on the relevant 
        ...
        mantain this property to all floorplan images. Figure \ref{fig:squaredBox} show an example of this 
        operation;
        \begin{figure}[h]
            \centering
            \begin{minipage}{0.92\textwidth}
        ...
        \end{figure}
    \item Image resizing: each floorplan image is resized to $400 \times 400$ pixels to 
...
training set:
\begin{itemize}
    \item Random flip: both horizontal and vertical flips  applied;
    \item Random rotation: a random rotation between $0^\circ,90^\circ,180^\circ,270^\circ$ is applied; 
\end{itemize}
...
Where $[0,255]^{n\times n}$ is a floorplan image, encoded as a $n\times n$ grayscale image, $\mathbb{R}^+$ is the
