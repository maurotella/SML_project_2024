...
from torch import Tensor, flatten, reshape
from torch.utils.data import DataLoader, Subset
from typing import List, Any, Optional
import torch.nn.init as init

def train(model, loss_fn, optimizer, epochs, train_loader, val_loader, device='cuda', verbose=True):
    ...
    for epoch in range(1,epochs+1):
        ...
        for x,area,ate,are in train_loader:
            optimizer.zero_grad()
            y_hat = model(x.to(device,non_blocking=True),area.to(device,non_blocking=True))
            y = stack([ate,are],dim=1).to(device,non_blocking=True).float()
        ...
        with no_grad(): 
            for xv,areav,atev,arev in val_loader: 
                y_hat = model(xv.to(device,non_blocking=True),areav.to(device,non_blocking=True))
                yv = stack([atev,arev],dim=1).to(device,non_blocking=True).float()
                val_loss = loss_fn(y_hat.float(), yv)
                running_vall_loss += val_loss.item()
            val_loss_value = running_vall_loss/len(val_loader)
        if verbose: print(f'\ttrain loss:\t {train_loss_value}\n\tval. loss:\t {val_loss_value}')
...
def test_error(model, testset, device="cpu", loss_fn=nn.MSELoss()):
    testloader = DataLoader(
        testset, batch_size=4, shuffle=False, num_workers=5
    )
    test_loss = 0.0
    model.to(device)
    model.eval()
...
def kCV(k, dataset, model, model_params, loss_fn, optimizer, opt_params:dict, epochs, batch_size, device='cuda'):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset)):
        ...
        l_fn = loss_fn()
        optim = optimizer(net.parameters(), **opt_params)
        ...
        epochs_loss = train(net,l_fn,optim,epochs,train_loader,val_loader,device)
        val_loss = epochs_loss['validation'][-1]
        all_loss += [epochs_loss]
        cv_losses += [val_loss]
        del net
    print(cv_losses)
    cv_loss = float(np.mean(cv_losses))
    return cv_loss
...
class AlexNet(nn.Module):
    def __init__(self, activation_function=nn.ReLU(), dropout=0.5):
        ...
        self.cnn = nn.Sequential(
            ...
            nn.Conv2d(256,384, kernel_size=3, padding=1),nn.BatchNorm2d(384), activation_function,
            nn.Conv2d(384,384, kernel_size=3, padding=1),nn.BatchNorm2d(384), activation_function,
            nn.Conv2d(384,256, kernel_size=3, padding=1),nn.BatchNorm2d(256), activation_function,
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.classifier = nn.Sequential(
            ...
            nn.BatchNorm1d(last_channel),   #last channel = 1024
            nn.Hardswish(inplace=True),
            nn.Dropout(p=dropout, inplace=True),
            nn.Linear(last_channel, 2), nn.ReLU()
        )
...
class MySqueezeNet(nn.Module):
    def __init__(self,dropout=0.5):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=3, stride=2), nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
            Fire(64, 16, 64, 64), nn.BatchNorm2d(128),
            Fire(128, 16, 64, 64), nn.BatchNorm2d(128),
            nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
            Fire(128, 32, 128, 128), nn.BatchNorm2d(256),
            Fire(256, 32, 128, 128), nn.BatchNorm2d(256),
            nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True),
            Fire(256, 48, 192, 192), nn.BatchNorm2d(384),
            Fire(384, 48, 192, 192), nn.BatchNorm2d(384),
            Fire(384, 64, 256, 256), nn.BatchNorm2d(512),
            Fire(512, 64, 256, 256), nn.BatchNorm2d(512),
        )
        final_conv = nn.Conv2d(512, 2, kernel_size=1)
        self.classifier = nn.Sequential(
            nn.Dropout(p=dropout), final_conv, nn.BatchNorm2d(2),
            nn.ReLU(inplace=True), nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten()
        )
        self.linear = nn.Sequential(
            nn.Linear(3,512),  nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(512,64), nn.ReLU(), nn.Dropout(dropout),
    ...
    def forward(self, x, area):
        x = self.features(x)
        x = self.classifier(x)
        combined = cat([x,area.view(-1,1).float()],dim=1)
